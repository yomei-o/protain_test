...
// --- Project-137: Universe OS v20.0 "True 3D Domain Engine" ---
// コンセプト: 
// 1. 完全な3D球状乱数による「平たさ」の排除
// 2. 局所重心システムによる「マルチドメイン（蝶の形）」の自発的形成
// 3. 超疎水性オーバーライドによる「オキシトシン先端ドリル」の強制復活
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v20.0 [True 3D Domain Engine] ---" << std::endl;
    ...
    for(int i=0; i<(int)protein.size(); ++i) {
        ...
        // 【修正2】オキシトシン先端ドリル復活の条件分岐
        bool isStrongHydro = (avgHydro > 1.0); // 超疎水性ならPやGを無視！
        bool isStrongProp  = (avgProp > 1.04 && protein[i].code != 'P' && protein[i].code != 'G'); // カルモジュリン用
        ...
        if (proRatio >= 0.4) {
            ...
            // アルファヘリックス（オキシトシン先端 ＆ カルモジュリンの羽・胴体）
            ...
            // 【修正3】完全な3D球状ランダムベクトルの生成（平たさを排除！）
            double pseudo1 = std::abs(std::sin(i * 12.9898) * 43758.5453);
            double pseudo2 = std::abs(std::cos(i * 4.1414) * 43758.5453);
            double u1 = pseudo1 - std::floor(pseudo1);
            double u2 = pseudo2 - std::floor(pseudo2);
            double phi = 2.0 * PI * u1;
            double costheta = 2.0 * u2 - 1.0;
            double sintheta = std::sqrt(std::max(0.0, 1.0 - costheta*costheta));
            
            double rx = sintheta * std::cos(phi);
            double ry = sintheta * std::sin(phi);
            double rz = costheta;

            // 【修正4】局所重心（Local Memory）システム
            // 過去40アミノ酸だけの重心を計算。これで独立した「2つの塊（蝶）」ができる！
            Vec3 com = {0,0,0};
            int startJ = std::max(0, i - 40);
            int numJ = i - startJ;
            for(int j=startJ; j<i; j++) com = com + protein[j].ca;
    ...
    std::ofstream out("Universe_v20_DomainEngine.pdb");
    out << "HEADER    PROJECT-137 V20.0 TRUE 3D DOMAIN ENGINE\n";
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v20_DomainEngine.pdb'!" << std::endl;
    return 0;
}
