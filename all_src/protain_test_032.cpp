...
// --- Project-137: Universe OS v21.0 "Domain-Splitter Engine" ---
// コンセプト: 
// 1. 長い螺旋による「ドメイン（塊）の分割」でカルモジュリンを蝶にする
// 2. 疎水性引力（マグネット効果）で、オキシトシンを隙間なくまとめる
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v21.0 [Domain-Splitter Engine] ---" << std::endl;
    ...
    for(int i=0; i<(int)protein.size(); ++i) {
        ...
        bool isStrongHydro = (avgHydro > 0.8); 
        bool isStrongProp  = (avgProp > 1.03 && protein[i].code != 'P' && protein[i].code != 'G');
        ...
        // 【ドメイン分割判定】長い螺旋(12残基以上)を抜けたら、新しい塊を作る！
        if (protein[i].isHelix || protein[i].isWire) {
            continuous_helix++;
        } else {
            if (continuous_helix >= 12) {
                domain_com = {0,0,0}; // 重心をリセット
                domain_count = 0;
                std::cout << "[Physics] Long helix ended. Starting new Domain at residue " << i+1 << "!" << std::endl;
            }
            continuous_helix = 0;
        }
        ...
        if (protein[i].isWire) {
            ...
            double hydroPull = (protein[i].hydrophobicity > -0.5) ? 3.0 : -0.5; // 強力な引力！
    ...
    for(int iter=0; iter<300; ++iter) {
        for(int i=0; i<(int)protein.size(); ++i) {
            for(int j=i+1; j<(int)protein.size(); ++j) {
                ...
                else if (dist >= MIN_DIST && dist < 12.0) {
                    // 【新機能】疎水性引力（遠くにある油同士が引き寄せ合ってまとまる！）
                    if (protein[i].hydrophobicity > 0.5 && protein[j].hydrophobicity > 0.5) {
                        double pull = 0.02; // マグネットの強さ
    ...
    std::ofstream out("Universe_v21_DomainSplitter.pdb");
    out << "HEADER    PROJECT-137 V21.0 DOMAIN SPLITTER ENGINE\n";
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v21_DomainSplitter.pdb'!" << std::endl;
    return 0;
}
