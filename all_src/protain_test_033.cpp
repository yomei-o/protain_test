...
// --- Project-137: Universe OS v22.0 "Soft-Body & Deep Collapse" ---
// コンセプト: 
// 1. 螺旋の判定を厳格化し、折り畳むための「関節」を復活させる。
// 2. 質量の概念を導入し、螺旋の形を保ったまま「玉」へと凝集させる。
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v22.0 [Soft-Body & Deep Collapse Engine] ---" << std::endl;
    ...
    for(int i=0; i<(int)protein.size(); ++i) {
        ...
        // 【修正1】螺旋の判定を極めて厳格化！これで「関節」が生まれる！
        bool isStrongHydro = (avgHydro > 1.6); // オキシトシンの真のドリル部分だけ
        bool isStrongProp  = (avgProp > 1.12 && protein[i].code != 'P' && protein[i].code != 'G'); // カルモジュリンの羽の中心部だけ
        ...
        if (protein[i].isHelix || protein[i].isWire) {
            ...
            if (continuous_helix >= 10) { // 10文字以上の螺旋でドメイン分割
                domain_com = {0,0,0}; 
                domain_count = 0;
            }
            continuous_helix = 0;
    ...
    // 【修正2】質量の概念を使ったソフトボディ・リラクゼーション
    for(int iter=0; iter<400; ++iter) { // 回数を増やして徹底的に丸める
        for(int i=0; i<(int)protein.size(); ++i) {
            for(int j=i+1; j<(int)protein.size(); ++j) {
                // 螺旋は重く（0.1）、関節は軽く（1.0）動く。完全固定はしない！
    ...
    std::ofstream out("Universe_v22_SoftBody.pdb");
    out << "HEADER    PROJECT-137 V22.0 SOFT-BODY ENGINE\n";
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v22_SoftBody.pdb'!" << std::endl;
    return 0;
}
