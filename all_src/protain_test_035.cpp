...
// --- Project-137: Universe OS v24.0 "Local Packing & Annealing" ---
// コンセプト: 
// 1. 引力の届く距離を 8.0 A に制限し、満員電車バグ（全方向からの引力相殺）を防ぐ。
// 2. 焼きなまし法（Annealing）を導入し、徐々に引力を強めて優しくパッキングする。
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v24.0 [Local Packing & Annealing] ---" << std::endl;
    ...
    // --- 2. 究極のリラクゼーション（Local Packing & Simulated Annealing） ---
    int total_iters = 600;
    for(int iter=0; iter<total_iters; ++iter) { 
        // 焼きなまし係数: 序盤は反発優先（0.0）、終盤に向けて優しく引力を強くする（1.0）
        ...
        for(int i=0; i<(int)protein.size(); ++i) {
            for(int j=i+1; j<(int)protein.size(); ++j) {
                ...
                if(dist < MIN_DIST) {
                    // 反発は常に全力で発動（Steric Shield）
                    double push = (MIN_DIST - dist) * 0.5;
                    if (dist < 3.0) push += (3.0 / dist); // 反発を少し強化
                ...
                // 【v24の魔法】引力の届く距離を「8.0 A以内」に制限し、遠くの原子を無理に引っ張らない
                else if (dist < 8.0) {
                    if (protein[i].hydrophobicity > 0.0 && protein[j].hydrophobicity > 0.0) {
                        // 【v24の魔法】焼きなまし（Annealing）で、徐々に引力を強くする
                        double pull = 0.015 * (protein[i].hydrophobicity + protein[j].hydrophobicity) * anneal_factor; 
                        if (pull > 0.2) pull = 0.2; // 引きすぎ防止
    ...
    std::ofstream out("Universe_v24_LocalPacking.pdb");
    out << "HEADER    PROJECT-137 V24.0 LOCAL PACKING & ANNEALING\n";
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v24_LocalPacking.pdb'!" << std::endl;
    return 0;
}
