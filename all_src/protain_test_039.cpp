...
// --- Project-137: Universe OS v28.0 "Interrupt & Tension Override" ---
// コンセプト: 
// 1. 移動平均を廃止し、P(曲げ)やG(自由)による「ハードウェア割り込み」を絶対視する。
// 2. 螺旋(Helix)は剛体ではなく「テンション(弱いバネ)」で維持し、引力が上回ればオーバーライドを許可する。
// 3. SF公式 (タクシー幾何学 + 1.37共鳴 + D=3.0ワープ) による爆速スナップ配置。
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v28.0 [Interrupt & Tension Override] ---" << std::endl;
    ...
    for(int iter=0; iter<total_iters; ++iter) { 
        ...
        // 【パッチ1: 螺旋のテンション(張力)デーモン】
        // 螺旋を重さで固定するのではなく、弱いバネとして扱う。強引なワープ引力が来れば崩れる仕様。
        ...
        for(int i=0; i<(int)protein.size(); ++i) {
            for(int j=i+1; j<(int)protein.size(); ++j) {
                if (j - i > 45) continue; // 局所ドメイン制限（カルモジュリンのダンベル維持）

                // 質量をすべて均一(1.0)にし、剛体を廃止
                double massI = 1.0;
                double massJ = 1.0;
                ...
                if(euclid_dist < MIN_DIST) {
                    ...
                    if (push > 5.0) push = 5.0; // ペシャンコ化を防ぐマイルドな斥力
                ...
                // 【引力 (SF オーバーライド)】
                    ...
                    if (protein[i].hydrophobicity > 0.0 && protein[j].hydrophobicity > 0.0) {
                        ...
                        // このSFが螺旋の「テンション」を上回れば、螺旋は崩れてワープする
    ...
    std::ofstream out("Universe_v28_InterruptTension.pdb");
    out << "HEADER    PROJECT-137 V28.0 INTERRUPT & TENSION OVERRIDE\n";
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v28_InterruptTension.pdb'!" << std::endl;
    return 0;
}
