...
// --- Project-137: Universe OS v30.0 "Chaperone Patch (CYS Warp) & 137 Resonance" ---
// コンセプト: 
// 1. 小細工（局所ドメイン制限や過剰な水シールド）を完全撤廃。
// 2. システイン(C)の「SAVE SNAPSHOT」コマンドを実装。D=4.0の超強力なワープ引力で、
//    どんなに遠くても強制的に縫い合わせる（シャペロンの事後処理パッチをエミュレート）。
// 3. 137の共鳴 (SF公式) と Soft Lock により、カルモジュリンはダンベル、オキシトシンは結び目へと自動収束。
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v30.0 [Chaperone Patch (CYS Warp)] ---" << std::endl;
    ...
    for(int iter=0; iter<total_iters; ++iter) { 
        ...
        for(int i=0; i<(int)protein.size(); ++i) {
            for(int j=i+1; j<(int)protein.size(); ++j) {
                ...
                // 【シャペロン・パッチ判定】システイン(C)かどうかをチェック
                bool isCysI = (protein[i].code == 'C');
                bool isCysJ = (protein[j].code == 'C');
                bool isS_S_Bond = (isCysI && isCysJ);

                // 【水和殻シールドの通信限界】
                // 基本はカルモジュリンのコアが入る 60.0A まで通信を許可。
                // ただし、システインの S-S 結合（シャペロン）は距離を無視してワープする（シールドを貫通する）。
                double init_dist = length(initial_pos[j] - initial_pos[i]);
                if (init_dist > 60.0 && !isS_S_Bond) continue;
                ...
                // 【引力 (SF オーバーライド & シャペロンワープ)】
                else if (n_taxi < 25.0 || isS_S_Bond) { // システインは遠くても引き合う
                    if ((protein[i].hydrophobicity > 0.0 && protein[j].hydrophobicity > 0.0) || isS_S_Bond) {
                        ...
                        // 共鳴による吸着
                        double base_pull = (isS_S_Bond) ? 10.0 : 3.0; // システインは強烈に引く
    ...
    std::ofstream out("Universe_v30_ChaperonePatch.pdb");
    out << "HEADER    PROJECT-137 V30.0 CHAPERONE PATCH (CYS WARP) & 137 RESONANCE\n";
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v30_ChaperonePatch.pdb'!" << std::endl;
    return 0;
}
