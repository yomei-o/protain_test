...
// --- Project-137: Universe OS v31.0 "Co-translational Water Shield & CYS Warp" ---
// コンセプト: 
// 1. 【Water Shield】OSは出力された端から水分子でパッキングする。45残基以上離れると絶縁され引力パケットは届かない。
//    -> これにより、カルモジュリンは2つのコアに分かれ「美しいダンベル」となる。
// 2. 【CYS Warp】ただし、システイン(C)の SAVE SNAPSHOT コマンドだけはこの水シールドを貫通してワープ結合する。
//    -> これにより、オキシトシンは距離を無視して強固に縫い合わされ「40Aの結び目」となる。
// 3. 【OS Command】P/Gによる螺旋割り込み、LIVハッシュによるD=3.0ワープ、1.37共鳴の完全統合。
...
double length(Vec3 v) { return std::sqrt(v.x*v.x + v.y*v.y + v.z*v.z); } // 物理的衝突・結合維持専用
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v31.0 [Water Shield & CYS Warp] ---" << std::endl;
    ...
    for(int i=0; i<(int)protein.size(); ++i) {
        if (protein[i].isWire) {
            ...
            // 3D空間へのふんわりとした自然な広がり（マージン）
            double hydroPull = (protein[i].hydrophobicity > 0) ? 1.0 : 0.0;
            
            axis.x += rx * 1.5 + axis.x * hydroPull;
            axis.y += ry * 1.5 + axis.y * hydroPull;
            axis.z += rz * 1.5 + axis.z * hydroPull;
            axis = normalize(axis);
    ...
    for(int iter=0; iter<total_iters; ++iter) { 
        ...
        for(int i=0; i<(int)protein.size(); ++i) {
            for(int j=i+1; j<(int)protein.size(); ++j) {
                ...
                // 【シャペロン・パッチ判定】
                bool isCysI = (protein[i].code == 'C');
                bool isCysJ = (protein[j].code == 'C');
                bool isS_S_Bond = (isCysI && isCysJ);

                // 【水分子コーティング（Water Shield）パッチ】
                // おじさんが見抜いたOSの仕様。作られた端から水でパッキングされ、
                // 45残基を超えると通信パケットは絶縁される。
                // ただし！システイン(C)の S-S 結合ワープだけはこのシールドを貫通する。
                if (j - i > 45 && !isS_S_Bond) continue;
                ...
                // 【引力 (SF オーバーライド & システイン貫通ワープ)】
                else if (n_taxi < 25.0 || isS_S_Bond) { 
                    if ((protein[i].hydrophobicity > 0.0 && protein[j].hydrophobicity > 0.0) || isS_S_Bond) {
                        
                        double D = 2.0; 
                        
                        // L, I, V (HASH_CORE_MAX系) はコア圧縮コマンド
                        ...
                        // 【シャペロン・パッチ】システインは D=4.0 の超強力な「SAVE SNAPSHOT」ワープを発動
                        if (isS_S_Bond) {
                            D = 4.0;
                        }
                        ...
                        // システインは上限なし（強制的にスナップ）、それ以外は 1.5 で制限
                        if (!isS_S_Bond && SF > 1.5) SF = 1.5; 
                        
                        // 共鳴による吸着
                        double base_pull = (isS_S_Bond) ? 10.0 : 3.0; // システインはシールド貫通で強烈に引く
                        double pull = SF * base_pull * (protein[i].hydrophobicity + protein[j].hydrophobicity) * anneal_factor; 
                        if (!isS_S_Bond && pull > 0.6) pull = 0.6; // 通常スナップの上限
    ...
    std::ofstream out("Universe_v31_FinalPatch.pdb");
    out << "HEADER    PROJECT-137 V31.0 CO-TRANSLATIONAL WATER SHIELD & CYS WARP\n";
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v31_FinalPatch.pdb'!" << std::endl;
    return 0;
}
