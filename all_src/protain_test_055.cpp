...
// --- Project-137: Universe OS v44.0 "Bio-Realism Final Engine" ---
// 生命のハードウェア仕様（リボソームトンネル）と物理法則（静電斥力）を完全実装。
// 1. 【リボソームトンネル (45残基)】: 
//    生成されたアミノ酸のうち、最新の45個はまだトンネル内にいるため、
//    遠距離の引力（疎水性崩壊）に参加できない（保護されている）。
//    これにより、カルモジュリンの前半と後半が時間差で独立して畳まれる。
// 2. 【静電斥力 (クーロン力)】:
//    電荷を持つアミノ酸（D, E, K, R）同士に電気的な力を働かせる。
//    カルモジュリンの玉同士（共にマイナス）は反発し、合体を防ぐ。
...
const int TUNNEL_LENGTH = 45; // ★リボソーム・トンネルの長さ（遅延バッファ）
...
AAProps getAAProps(char code) {
    static std::map<char, AAProps> m = {
        // Charge: D,E = -1; K,R = +1; Others = 0 (Simplified)
        ...
        {'V',{"VAL", 4.2, 1.06, 0}}, {'D',{"ASP",-3.5, 1.01, -1}}, {'H',{"HIS",-3.2, 1.00, 0.5}}, // His is weak +
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v44.0 [Bio-Realism Final] ---" << std::endl;
    ...
    for (int step = 0; step < TOTAL_STEPS; step++) {
        ...
        // 6. 【長距離相互作用】(疎水性引力、斥力、静電斥力、ワープ)
        // ★重要：リボソームトンネル内の粒子は、この「遠距離の力」の影響を受けない★
        // つまり、spawned_count - TUNNEL_LENGTH より昔に生まれた粒子だけが参加できる
        ...
        for (int i = 0; i < tunnel_exit_index; i++) {
            for (int j = i + 5; j < tunnel_exit_index; j++) {
                ...
                // (C) 静電斥力 [★v44.0 新パッチ★]
                // 同じ符号の電荷は反発し、違う符号は引き合う
                if (d < 25.0 && std::abs(p[i].charge) > 0.1 && std::abs(p[j].charge) > 0.1) {
                    double electric = (p[i].charge * p[j].charge) * 20.0 / (d * d); // クーロンの法則的
                    // マイナス同士ならプラスの力（反発）になる
                    force_mag -= electric; 
                }
    ...
    std::ofstream out("Universe_v44_BioRealism.pdb");
    out << "HEADER    PROJECT-137 V44.0 BIO-REALISM FINAL ENGINE\n";
    int atomId = 1;
    for (int i=0; i<N; i++) {
        char buf[100];
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v44_BioRealism.pdb'!" << std::endl;
    return 0;
}
