...
// --- Project-137 v45.0 "Interaction Horizon (局所相互作用の優先)" ---
// 「リンカーをif文で緩める」というインチキを排除。
// 代わりに「近くの仲間とは強く引き合い、遠くとはあまり引き合わない」という
// 物理的な「相互作用の地平線（Horizon）」を導入する。
// これにより、疎水性残基が密集している両端は勝手に丸まり、
// 密集していない真ん中（リンカー）は勝手に一本の紐として残る（創発する）。
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v45.0 [Interaction Horizon] ---" << std::endl;
    ...
    int N = p.size();
    int TOTAL_STEPS = (N * STEPS_PER_AA) + 15000; // 時間をたっぷり取って、ゆっくり丸まらせる
    ...
    for (int step = 0; step < TOTAL_STEPS; step++) {
        ...
        if (step % STEPS_PER_AA == 0 && spawned_count < N) {
            ...
            if (spawned_count > 0) {
                ...
                // まだ丸まっていない状態でまっすぐ送り出す
                p[spawned_count].pos = prev_pos + Vec3{3.0, dis(gen)*0.5, dis(gen)*0.5}; 
            } else {
        ...
        // トンネルから出た粒子（tunnel_exit_index以前）について計算
        ...
        for (int i = 0; i < tunnel_exit_index; i++) {
            for (int j = i + 5; j < tunnel_exit_index; j++) {
                ...
                // (B) 疎水引力 [★v45.0 修正★]
                // 距離が近い（シーケンス上の距離が近い）ほど、猛烈に引き合う。
                // 遠い（シーケンス的に離れている）と、水シールドで引力が弱まる。
                // これにより、局所的な「玉」ができやすくなり、全体が潰れるのを防ぐ。
                else if (d < 30.0 && p[i].hydro > 0 && p[j].hydro > 0) {
                    int seq_dist = std::abs(i - j);
                    double local_bias = 1.0;
                    
                    if (seq_dist < 25) {
                        local_bias = 3.0; // 近くの仲間とは強く抱き合う（玉を作る）
                    } else {
                        local_bias = 0.5; // 遠くの仲間とはあまり関わらない（独立性を保つ）
                    }
    ...
    std::ofstream out("Universe_v45_InteractionHorizon.pdb");
    out << "HEADER    PROJECT-137 V45.0 INTERACTION HORIZON ENGINE\n";
    int atomId = 1;
    for (int i=0; i<N; i++) {
        char buf[100];
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v45_InteractionHorizon.pdb'!" << std::endl;
    return 0;
}
