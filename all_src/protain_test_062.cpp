...
// --- Project-137: Universe OS v48.0 "Electro-Expansion" ---
// ベース: v43.0 (リボソーム・エミュレータ)
// 修正: カルモジュリンの中央リンカーにある「Glu-Glu-Glu」などの
//       「同符号の電荷の連続」に着目。
//       隣り合うアミノ酸同士が電気的に反発することで、
//       鎖が縮こまらずにピンと伸びる（伸長する）メカニズムを実装。
//       これにより、硬いバネを使わずとも、自然にダンベル型が維持される。
//       一方、プレプロオキシトシンは強力な「システイン結合」を持つため、
//       この程度の反発力では崩れず、丸い形を保つことができる。
...
int main() {
    std::string rawInput;
    std::cout << "--- Project-137 v48.0 [Electro-Expansion Engine] ---" << std::endl;
    ...
    for (int step = 0; step < TOTAL_STEPS; step++) {
        ...
        // ここがv48.0の核心
        for (int i = 0; i < spawned_count; i++) {
            // (A) 隣接するアミノ酸同士の静電反発（伸長力）
            // i と i+1, i+2, i+3 などの「近いけど結合バネではない」範囲
            for (int k = 1; k <= 3; k++) {
                int j = i + k;
                if (j >= spawned_count) break;
                
                // 同じ符号の電荷なら、強く反発させて「真っ直ぐ」にする
                // これはカルモジュリンの中央（Glu-Glu-Glu）で特に強く働く
                if (std::abs(p[i].charge) > 0.1 && std::abs(p[j].charge) > 0.1 && 
                    (p[i].charge * p[j].charge > 0)) {
                    
                    Vec3 dir = p[j].pos - p[i].pos;
                    double d = length(dir);
                    if (d < 10.0) { // 近距離のみ
                        double repel = 100.0 / (d * d); // 強力な反発
                        Vec3 f = normalize(dir) * repel;
                        p[i].force -= f;
                        p[j].force += f;
                    }
                }
            ...
            for (int j = i + 5; j < spawned_count; j++) {
                ...
                // 長距離の静電斥力（全体の形を整える）
                // オキシトシンのシステイン結合(40.0)に比べれば弱いので、オキシトシンは崩れない
                if (d < 30.0 && std::abs(p[i].charge) > 0.1 && std::abs(p[j].charge) > 0.1) {
                     double electric = (p[i].charge * p[j].charge) * 30.0 / (d * d);
                     force_mag -= electric;
                }

                // シャペロンワープ（システイン結合）
                // これが最強の引力なので、オキシトシンは必ず丸まる
    ...
    std::ofstream out("Universe_v48_ElectroExpansion.pdb");
    out << "HEADER    PROJECT-137 V48.0 ELECTRO-EXPANSION ENGINE\n";
    int atomId = 1;
    for (int i=0; i<N; i++) {
        char buf[100];
    ...
    out << "END\n";
    std::cout << "[Complete] Check 'Universe_v48_ElectroExpansion.pdb'!" << std::endl;
    return 0;
}
